...
s:^\s*[#]*(greeter-setup-script=).*:\1/usr/share/lightdm/scripts/aw-greeter-setup.sh:" \
/etc/lightdm/lightdm.conf

#
# Enable auto managers
if [ -n "$S_AUTODISPLAY" ]; then
  sudo sed -i -E "s/^\s*[#]*(AUTODISPLAY.*)/\1/" /usr/share/lightdm/scripts/aw-display-setup.sh
fi
if [ -n "$S_AUTOWALLPAPER" ]; then
  sudo sed -i -E "s/^\s*[#]*(CHECKWALLPAPER.*)/\1/" /usr/share/lightdm/scripts/aw-greeter-setup.sh
fi
